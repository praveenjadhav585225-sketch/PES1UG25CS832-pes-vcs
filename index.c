...
int index_load(Index *index) {
    ...
    while (fgets(line, sizeof(line), f) != NULL) {
        ...
        int parsed = sscanf(line, "%15s %64s %llu %u %511[^\n]",
                            mode_str, hash_hex, &mtime_ull, &size_u, path);
        if (parsed != 5) {
            fclose(f);
            return -1;
        }
...
int index_add(Index *index, const char *path) {
    ...
    if ((off_t)len != st.st_size) return -1;
    ...
    e->mode = (st.st_mode & S_IXUSR) ? MODE_EXEC : MODE_FILE;
    e->hash = blob_id;
